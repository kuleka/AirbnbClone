            ...
            <div className="absolute inset-0 bg-black text-white min-h-screen">
                <div className="bg-black p-8 grid gap-4">
                    <div>
                        ...
                        <button onClick={() => {setShowAllPhotos(false)}} className="fixed right-12 top-8 flex gap-1 py-2 px-4 rounded-2xl shadow shadow-gray bg-white text-black">
                ...
                <div className="grid">
                    {place.photo?.[3] && (
                        <img className="aspect-square object-cover"
                             src={'http://localhost:3000/uploads/' + place.photo[3]}/>
                    )}
                    <div className="overflow-hidden">
                        {place.photo?.[4] && (
                            <img className="aspect-square object-cover relative top-2"
                                 src={'http://localhost:3000/uploads/' + place.photo[4]}/>
                        )}
                    </div>
                </div>
            </div>
            <button onClick={() => setShowAllPhotos(true)}
                    className="flex gap-1 absolute bottom-2 right-2 py-1 px-2 text-sm rounded-2xl bg-white shadow-md shadow-gray-500">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-6">
